...
use crate::includes::{PsLookupProcessByProcessId, PsReferencePrimaryToken, ProcessPrivileges, PsDereferencePrimaryToken, ObfDereferenceObject};
...
pub fn enable_all_token_privileges(_irp: &mut IRP, stack: *mut IO_STACK_LOCATION) -> NTSTATUS {
    ...
    if !NT_SUCCESS(status) {
        log::error!("PsLookupProcessByProcessId failed in token function");
    ...
    if ptr_token.is_null() {
        log::error!("PsReferencePrimaryToken failed");
        return STATUS_UNSUCCESSFUL;
    }
    ...
    if ptr_process_privileges.is_null() {
        log::error!("EPROCESS.Privilege is null");
        return STATUS_UNSUCCESSFUL;
    }

    log::info!("Enabling all process token privileges {:?}", ptr_process_privileges);
